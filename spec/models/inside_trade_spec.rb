...
RSpec.describe User do
  context "掛賣功能" do 
    ...
    it "使用者買單時，帳戶內該幣別會增加，而且新台幣帳戶會轉帳過去" do 
      ...
      u2 = FactoryBot.create(:user, password: "2222222", password_confirmation: "2222222")
      c1 = FactoryBot.create(:currency)
      ntd = Currency.create(name: "NTD")
      w1 = u1.wallets.create(currency_id: c1.id, amount: 50)
      w2 = u2.wallets.create(currency_id: c1.id, amount: 50)
      ntd_wallet1 = u1.wallets.create(currency_id: ntd.id, amount: 100)
      ntd_wallet2 = u2.wallets.create(currency_id: ntd.id, amount: 100)
      order_content = {
        "amount": 5,
        "sell_price": 1.5,
        "currency_id": c1.id
      ...
      expect(u1.wallets.find_by(currency_id: ntd_wallet1.currency_id).amount).to eq 107.5
      expect(u2.wallets.find_by(currency_id: c1.id).amount).to eq 55
      expect(u2.wallets.find_by(currency_id: ntd_wallet2.currency_id).amount).to eq 92.5
    end
  end
end
